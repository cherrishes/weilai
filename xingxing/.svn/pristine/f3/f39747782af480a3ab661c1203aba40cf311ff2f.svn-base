...
LineProtocol = function (transport) {
    ...
    transport.onread = function (data) {
        log.debug("transport.onread: enter isLineMode=", isLineMode,
            " buffer[", buffer.length, "]=", buffer, " data[", data.length,
            "]=", data);
        ...
        if (isLineMode) {
            ...
            if (isLineMode) {
                // TODO if this buffer length is above a given threshold, we
                // should
                // send an alert "max line length exceeded" and empty buffer
                // or even abort.
            } else {
                // we've left the line mode and what remains in buffer is raw
                // data.
                data = buffer;
                buffer = "";
            }
...
STOMPClient = function () {
    ...
    function doDispatch(bytes, extra) {
        log.debug("doDispatch: bytes[", bytes.length, "]=", bytes, " extra[",
            extra.length, "]=", extra);
    ...
    self.send = function (message, destination, extraHeaders) {
        self.sendFrame("SEND", mergeObject({
            destination: destination
        }, extraHeaders), message);
    };

    self.subscribe = function (destination, extraHeaders) {
        self.sendFrame("SUBSCRIBE", mergeObject({
            destination: destination
        }, extraHeaders));
    };

    self.unsubscribe = function (destination, extraHeaders) {
        self.sendFrame("UNSUBSCRIBE", mergeObject({
            destination: destination
        }, extraHeaders));
    };

    self.begin = function (id) {
        self.sendFrame("BEGIN", {
            "transaction": id
        });
    };

    self.commit = function (id) {
        self.sendFrame("COMMIT", {
            "transaction": id
        });
    };

    self.abort = function (id) {
        self.sendFrame("ABORT", {
            "transaction": id
        });
    };
