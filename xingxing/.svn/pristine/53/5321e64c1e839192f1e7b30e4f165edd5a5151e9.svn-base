...
(function () {
    ...
    (function () {
        ...
        if (window.btoa && window.btoa('1') == 'MQ==') {
            Orbited.base64.encode = function (data) {
                return btoa(data);
            };
            Orbited.base64.decode = function (data) {
                return atob(data);
            };
            return;
        }
    ...
    // TODO: is it confusing to have Orbited.Loggers be the various logging
    // classes
    ...
    Orbited.util.chooseTransport = function () {
        ...
        // TODO: sort the choices by the values of
        // transport[Orbited.util.browser]
    ...
    Orbited.CometSession = function () {
        ...
        /*
         * This will always fire same-domain and cross-subdomain. It will fire
         * most of the time cross-port, but it's not strictly guaranteed. -mario
         */
        var hardClose = function () {
            var tdata = encodePackets([[++packetCount, "close"]]);
            if (xsdClose) {
                xsdClose.contentWindow.sendCloseFrame(sessionUrl.render(),
                    tdata);
            } else {
                xhr.open('POST', sessionUrl.render(), !sessionUrl
                    .isSameDomain(location.href));
                xhr.send(tdata);
            }
        }

        /*
         * self.open can only be used when readyState is INITIALIZED.
         * Immediately following a call to self.open, the readyState will be
         * OPENING until a connection with the server has been negotiated.
         * self.open takes a url as a single argument which desiginates the
         * remote url with which to establish the connection.
         */
        self.open = function (_url) {
            ...
            if (sessionUrl.isSameDomain(location.href)) {
                ...
                if (sessionUrl.isSamePort(location.href)) {
                    ...
                    ifUrl.path = '/static/orbited/xsdClose.html';
        ...
        /*
         * self.send is only callable when readyState is OPEN. It will queue the
         * data up for delivery as soon as the upstream xhr is ready.
         */
        ...
        /*
         * self.close sends a close frame to the server, at the end of the
         * queue. It also sets the readyState to CLOSING so that no further data
         * may be sent. onclose is not called immediately -- it waits for the
         * server to send a close event.
         */
        self.close = function () {
        ...
        /*
         * self.reset is a way to close immediately. The send queue will be
         * discarded and a close frame will be sent to the server. onclose is
         * called immediately without waiting for a reply from the server.
         */
        self.reset = function () {
            ;
            ;
        ...
        var transportOnReadFrame = function (frame) {
            ...
            ;
            ;
            self.logger.debug('READ FRAME: ', frame.id, frame.name,
            ...
            switch (frame.name) {
                ...
                case 'data':
                    ...
                    ;
                    ;
                    self.logger.debug('base64 decoding ' + frame.data.length
                    + ' bytes of data');
        ...
        var doSend = function (retries) {
            ...
            // NB: its awkard, but for reusing the XHR object in IE (7 at
            // least),
        ...
        var resetTimeout = function () {
            ...
            unsetTimeout();
            timeoutTimer = window.setTimeout(timedOut, pingInterval
            + pingTimeout);
        };
    ...
    Orbited.CometSession.prototype.logger = Orbited
        .getLogger("Orbited.CometSession");
    ...
    Orbited.test.stompdispatcher.send = function (dest, msg) {
        ...
        ;
        ;
        Orbited.test.logger.debug("stompdispatcher dispatching " + msg + " to "
        + dest);
        var s = document.createElement('script');
        s.src = "http://" + Orbited.settings.hostname + ":"
        + Orbited.settings.port + "/system/test/stomp?";
        s.src += "msg=" + msg;
        s.src += "&dest=" + dest;
    ...
    Orbited.TCPSocket = function () {
        ...
        /*
         * self.open attempts to establish a tcp connection to the specified
         * remote hostname on the specified port. When specified as true, the
         * optional argument, isBinary, will cause onread to return byte arrays,
         * and send will only accept a byte array.
         */
        ...
        /*
         * self.reset closes the connection from this end immediately. The
         * server may be notified, but there is no guarantee. The main purpose
         * of the reset function is for a quick teardown in the case of a user
         * navigation. if reset is not called when IE navigates, for instance,
         * there will be potential issues with future TCPSocket communication.
         */
        self.reset = function () {
            if (session) {
                session.reset();
        ...
        var doClose = function (code) {
            ...
            if (session) {
                if (code == Orbited.Statuses.ServerClosedConnection
                    || code == Orbited.Errors.Unauthorized
                    || code == Orbited.Errors.RemoteConnectionFailed) {
                    session.cleanup();
                } else {
        ...
        var receive = function (payload) {
            ...
            switch (payload[0]) {
                case 'initialized':
                    ...
                    ;
                    ;
                    self.logger.debug('Orbited.singleton.XSDR.queues[id] is',
                        Orbited.singleton.XSDR.queues[id]);
                    break;
    ...
    if (Orbited.util.browser == "opera") {
        ...
        (window.postMessage && window || document).addEventListener('message',
            function (e) {
                ...
                if (cmd == "queues") {
                    ...
                    if (queue.length > 0) {
                        data = queue.shift();
                        Orbited.singleton.XSDR.iframes[id][pmLocation]
                            .postMessage(Orbited.JSON.stringify(data),
                            e.origin);
                    }
                }
            }, false);
                    ...
                    switch (xhr.readyState) {
                        ...
                        case 4:
                            ...
                            if (doReconnect) {
                                ...
                                window.clearTimeout(heartbeatTimer);
                                retryTimer = window.setTimeout(reconnect,
                                    retryInterval);
                                return;
                            }
                            switch (xhr.status) {
                                case 200:
                                    ...
                                    // }
                                    // alert('a');
                                    // alert('stream over ' + typeof(console) + ' ' +
                                    // typeof(Orbited) + ' ' + Orbited + ' ...');
                                    process();
                                    offset = 0;
                                    setTimeout(open, 0);
                                    window.clearTimeout(heartbeatTimer);
    ...
    Orbited.CometTransports.XHRStream.prototype.logger = Orbited
        .getLogger("Orbited.CometTransports.XHRStream");
                    ...
                    switch (xhr.readyState) {
                        case 4:
                            try {
                                ...
                                ;
                                self.logger
                                    .debug("start reconnect Timer (couldn't access xhr.status)");
                                retryInterval *= 2;
                            ...
                            switch (xhr.status) {
                                case 200:
                                    ...
                                    ;
                                    self.logger
                                        .debug("completed request, reconnect immediately");
                                    setTimeout(open, 0);
                                    break;
                                case 404:
                                    self.close();
                                ...
                                case null:
                                    ...
                                    ;
                                    self.logger
                                        .debug("start reconnect Timer (null xhr.status)");
                                    window.setTimeout(reconnect, retryInterval);
                                    break;
                                default:
                                    ...
                                    ;
                                    ;
                                    self.logger.debug("something broke, xhr.status=",
                                        xhr.status);
                                    self.close();
                                    break;
                            }
        ...
        var process = function () {
            ...
            while (true) {
                ...
                if (commaPos == -1) {
                    ...
                    ;
                    ;
                    self.logger.debug('no more commas. offset:', offset,
                ...
                /*
                 * if (stream.length < argEnd) { //;;; self.logger.debug('how
                 * did we get here? stream.length:', stream.length, 'argEnd:',
                 * argEnd, 'offset:', offset) return }
                 */
                var data = stream.slice(commaPos + 1, argEnd);
                ;
                ;
                ;
                self.logger.assert(data.length == argSize, 'argSize:', argSize,
    ...
    Orbited.CometTransports.LongPoll.prototype.logger = Orbited
        .getLogger("Orbited.CometTransports.LongPoll");
    ...
    Orbited.CometTransports.Poll.prototype.logger = Orbited
        .getLogger("Orbited.CometTransports.Poll");

    // Poll supported browsers
    /*
     * Orbited.CometTransports.Poll.firefox = 0.5
     * Orbited.CometTransports.Poll.opera = 0.5 Orbited.CometTransports.Poll.ie =
     * 0.5
     */
    ...
    Orbited.CometTransports.HTMLFile = function () {
        ...
        var doOpen = function (_url) {
            ...
            htmlfile = new ActiveXObject('htmlfile'); // magical microsoft
            // object
            htmlfile.open();
            if (self.isSubDomain) {
                htmlfile.write('<html><script>' + 'document.domain="'
                + document.domain + '";' + '</script></html>');
            } else {
    ...
    Orbited.CometTransports.HTMLFile.prototype.logger = Orbited
        .getLogger("Orbited.CometTransports.HTMLFile");
    ...
    Orbited.CometTransports.SSE = function () {
        ...
        self.close = function () {
            ...
            // TODO: can someone test this and get back to me? (No opera at the
            // moment)
            // : -mcarter 7-26-08
            self.readyState = CT_READYSTATE_CLOSED;
            doClose();
        ...
        doOpen = function () {
            /*
             * if (typeof(lastEventId) == "number") { url.setQsParameter('ack',
             * lastEventId) }
             */
         ...
         * window.clearTimeout(heartbeatTimer); heartbeatTimer =
         * window.setTimeout(reconnect, Orbited.settings.HEARTBEAT_TIMEOUT) }
         */
        var receive = function (id, name, data) {
            var tempId = parseInt(id);
            if (!isNaN(tempId)) {
                // NOTE: The old application/x-dom-event-stream transport
                // doesn't
    ...
    Orbited.CometTransports.SSE.prototype.logger = Orbited
        .getLogger("Orbited.CometTransports.SSE");
     ...
     * -mcarter, 7-30-08
     * 
     * Jacob is actually throwing this away and rewriting from scratch -mcarter
     * 11-14-08
     */
    Orbited.URL = function (_url) {
        ...
        self.render = function () {
            ...
            output += self.domain;
            if (self.port != 80 && typeof (self.port) != "undefined"
                && self.port != null)
        ...
        self.isSubDomain = function (_url) {
            ...
            return (_url.port == self.port && self.domain.indexOf("."
            + _url.domain) > 0);
            // return (_url.port == self.port && _url.domain ==
            // self.domain.split('.').slice(1).join('.'));
        };
    ...
    Orbited.utf8.decode = function (s) {
        ...
        for (var i = 0; i < s.length; i++) {
            if ((s.charCodeAt(i) & 0xf8) == 0xf0) {
                ...
                j += 4;
                ret.push(String.fromCharCode(parseInt((s.charCodeAt(i) & 0x07)
                    .toString(2)
                + pad6((s.charCodeAt(i + 1) & 0x3f).toString(2))
                + pad6((s.charCodeAt(i + 2) & 0x3f).toString(2))
                + pad6((s.charCodeAt(i + 3) & 0x3f).toString(2)), 2)));
                i += 3;
                ...
                j += 3;
                ret.push(String.fromCharCode(parseInt((s.charCodeAt(i) & 0x0f)
                    .toString(2)
                + pad6((s.charCodeAt(i + 1) & 0x3f).toString(2))
                + pad6((s.charCodeAt(i + 2) & 0x3f).toString(2)), 2)));
                i += 2;
                ...
                j += 2;
                ret.push(String
                    .fromCharCode(parseInt((s.charCodeAt(i) & 0x1f)
                        .toString(2)
                    + pad6(
                        (s.charCodeAt(i + 1) & 0x3f)
                            .toString(2), 6), 2)));
                i += 1;
            } else {
                j += 1;
    ...
    /*
     * We create Orbited.JSON whether or not some other JSON exists. This is
     * because Orbited.JSON is compatible with JSON.js (imported by xsdrBridge),
     * whereas various other JSONs, including the one that ships with Prototype,
     * are not, leading to dumb errors. -mario
     */
    ...
    Orbited.JSON = function () {
        ...
        var cx = /[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g, escapeable = /[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g, gap, indent, meta = { // table
            // of
            // character
            // substitutions
            '\b': '\\b',
